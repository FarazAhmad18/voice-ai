    ...
    <div className="bg-white rounded-lg border border-slate-200 overflow-hidden">
      <div className="overflow-y-auto" style={{ maxHeight: 560 }}>
        <div className="grid relative" style={{ gridTemplateColumns: '72px 1fr', height: totalHeight }}>
          {/* Time gutter */}
          <div className="border-r border-slate-100 relative bg-slate-50/50">
            {hours.map(({ label, hour }) => (
              <div key={hour} className="absolute right-3 -translate-y-1/2 text-[10px] text-slate-400 font-medium whitespace-nowrap" style={{ top: (hour - START_HOUR) * HOUR_HEIGHT }}>
                {label}
              </div>
            ))}
          </div>
          ...
          <div className="relative">
            {/* Hour lines */}
            {hours.map(({ hour }) => (
              <div key={hour} className="absolute left-0 right-0 border-t border-slate-100" style={{ top: (hour - START_HOUR) * HOUR_HEIGHT }} />
            ))}
            {hours.map(({ hour }) => (
                  ...
                  <div className="px-3 py-2">
                    <div className="flex items-center justify-between gap-2">
                      <div className="flex items-center gap-2 min-w-0">
                        ...
                        <span className="text-xs font-semibold text-slate-800 truncate">{apt.caller_name}</span>
                      </div>
