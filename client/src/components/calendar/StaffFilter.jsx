    ...
    <div className="relative" ref={ref}>
      <button
        ...
        className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-medium border transition-colors ${
          open ? 'bg-slate-100 border-slate-300' : 'border-slate-200 hover:bg-slate-50'
        }`}
      >
        {selectedStaff ? (
          <>
            <span className={`w-2 h-2 rounded-full ${selectedColor.dot}`} />
            <span className="text-slate-700">{selectedStaff.name}</span>
          </>
        ) : (
          <>
            <Users size={14} className="text-slate-400" />
            <span className="text-slate-500">All Staff</span>
          </>
        )}
      ...
      {open && (
        <div className="absolute top-full right-0 mt-1 w-56 bg-white rounded-lg border border-slate-200 shadow-lg z-50 overflow-hidden">
          <button
            onClick={() => { onSelect(null); setOpen(false); }}
            className={`w-full flex items-center gap-3 px-3 py-2.5 text-sm hover:bg-slate-50 transition-colors ${
              !selectedStaffId ? 'bg-slate-50 text-slate-900' : 'text-slate-600'
            }`}
          >
            <Users size={14} className="text-slate-400" />
            ...
            return (
              ...
              >
                ...
                <div className="flex-1 text-left min-w-0">
                  <p className="text-slate-700 truncate">{s.name}</p>
                  {s.specialization && <p className="text-[10px] text-slate-400 truncate">{s.specialization}</p>}
                </div>
