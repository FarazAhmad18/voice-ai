...
export default function AppointmentDetailPanel({ appointment, staffMap, onClose, onUpdateStatus }) {
  ...
  return (
    <>
      ...
      {/* Panel */}
      <div className="fixed right-0 top-0 bottom-0 w-full max-w-sm bg-white shadow-xl z-50 flex flex-col border-l border-slate-200">
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
          <div>
            <p className="text-lg font-semibold text-slate-900">{appointment.appointment_time}</p>
            <p className="text-xs text-slate-400 mt-0.5">{appointment.appointment_date}</p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-5">
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-semibold text-slate-900">{appointment.caller_name || 'Unknown'}</p>
          ...
          <div className="space-y-3">
            {appointment.case_type && (
              <div className="flex items-center gap-3 text-sm">
                ...
                <span className="text-slate-600 capitalize">{appointment.case_type}</span>
              </div>
            )}
            <div className="flex items-center gap-3 text-sm">
              <Clock size={14} className="text-slate-300 flex-shrink-0" />
              <span className="text-slate-600">{appointment.appointment_date} at {appointment.appointment_time}</span>
            </div>
            {appointment.urgency && appointment.urgency !== 'low' && (
              <div className="flex items-center gap-3 text-sm">
                <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${
                  appointment.urgency === 'high' ? 'bg-red-50 text-red-600' : 'bg-amber-50 text-amber-600'
                }`}>
            ...
            <div className="bg-slate-50 rounded-lg p-3">
              ...
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  {staff.specialization && <p className="text-xs text-slate-400">{staff.specialization}</p>}
                </div>
              </div>
            </div>
          )}
          ...
          {appointment.lead_id && (
            ...
            >
              <span className="text-sm font-medium text-slate-600 group-hover:text-slate-900">View Lead Details</span>
        ...
        {appointment.status === 'confirmed' && (
          <div className="px-5 py-4 border-t border-slate-100 flex gap-3">
            ...
            <button
              ...
              disabled={!!updating}
              className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 bg-white text-red-600 text-sm font-medium rounded-lg border border-red-200 hover:bg-red-50 transition-colors disabled:opacity-50"
            >
