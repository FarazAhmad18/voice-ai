                ...
                <div className="flex items-center justify-between px-5 py-4 cursor-pointer hover:bg-slate-50/50 transition-colors"
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-400 capitalize">{t.industry} · {t.variables?.length || 0} variables · {t.body?.length || 0} chars</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    <button onClick={(e) => { e.stopPropagation(); openPreview(t); }}
                      className="p-2 text-slate-400 hover:text-violet-600 transition-colors" title="Preview rendered output">
                      <Eye size={14} />
                    </button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] flex flex-col overflow-hidden">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
              <div>
                <h3 className="text-sm font-semibold text-slate-800">Prompt Preview</h3>
                <p className="text-xs text-slate-400 mt-0.5">{previewTemplateData.name}</p>
              </div>
              <button onClick={closePreview} className="p-1.5 text-slate-400 hover:text-slate-600 rounded-lg hover:bg-slate-100 transition-colors">
                <X size={16} />
              </button>
            </div>
            ...
            <div className="px-6 py-4 border-b border-slate-100 bg-slate-50/50">
              <div className="flex items-end gap-3">
                <div className="flex-1">
                  <label className="block text-xs font-medium text-slate-500 mb-1.5">Render for client</label>
                  {firms.length === 0 ? (
                    <p className="text-xs text-slate-400 italic">No clients yet — create a client first to preview</p>
                  ) : (
                    <select
                      value={previewFirmId}
            ...
            <div className="flex-1 overflow-y-auto px-6 py-4">
              ...
              {!previewResult && !previewError && !previewLoading && (
                <div className="flex flex-col items-center justify-center py-16 text-center">
                  <Eye size={28} className="text-slate-200 mb-3" />
                  <p className="text-sm text-slate-400">Select a client and click Render Preview</p>
                  <p className="text-xs text-slate-300 mt-1">Shows exactly what gets pushed to Retell on sync</p>
                </div>
              )}
                ...
                <div className="space-y-4">
                  ...
                  <div className="relative">
                    <div className="flex items-center justify-between mb-2">
                      <p className="text-xs font-medium text-slate-500">Rendered for <span className="text-slate-700">{previewResult.firm_name}</span></p>
                      <button
                        onClick={copyPrompt}
                        className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-slate-500 hover:text-slate-700 bg-slate-100 hover:bg-slate-200 rounded-lg transition-colors">
