...
const LEVEL_COLORS = {
  ...
  debug: { text: 'text-slate-500', badge: 'bg-slate-500/20 text-slate-500 ring-slate-500/30' },
};
        ...
        <div className="sticky top-[60px] z-30 bg-[#161b22] border-b border-[#30363d] px-5 py-3.5">
          <div className="flex items-center justify-between max-w-7xl mx-auto">
            <div className="flex items-center gap-3">
              ...
              <h1 className="text-[15px] font-bold text-slate-200">Process Logs</h1>
              <div className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-medium ${
                live ? 'bg-emerald-500/15 text-emerald-400' : 'bg-slate-500/15 text-slate-500'
              }`}>
            ...
            <div className="flex items-center gap-1.5">
              <button
                ...
                className={`w-8 h-8 rounded-lg flex items-center justify-center transition-colors ${
                  live ? 'bg-emerald-500/20 text-emerald-400 hover:bg-emerald-500/30' : 'bg-[#21262d] text-slate-400 hover:text-slate-300 hover:bg-[#30363d]'
                }`}
              ...
              <button
                ...
                className="w-8 h-8 rounded-lg bg-[#21262d] text-slate-400 hover:text-slate-300 hover:bg-[#30363d] flex items-center justify-center transition-colors"
                title="Refresh"
              >
              ...
              <button
                ...
                className="w-8 h-8 rounded-lg bg-[#21262d] text-slate-400 hover:text-red-400 hover:bg-[#30363d] flex items-center justify-center transition-colors"
                title="Clear view"
              >
                <Trash2 size={14} />
              </button>
          ...
          <div className="flex items-center gap-2.5 max-w-7xl mx-auto">
                ...
                <div className="absolute top-full left-0 mt-1 w-52 bg-[#1c2128] border border-[#30363d] rounded-xl shadow-2xl z-50 overflow-hidden">
                  <button
                    ...
                    className={`w-full text-left px-3 py-2 text-xs font-medium transition-colors ${
                      category === 'all' ? 'bg-blue-500/15 text-blue-400' : 'text-slate-300 hover:bg-[#21262d]'
                    }`}
                  >
                    All Processes
                  </button>
                  {Object.entries(CATEGORY_GROUPS).map(([group, cats]) => (
                    <div key={group}>
                      <div className="px-3 py-1.5 text-[10px] font-bold text-slate-500 uppercase tracking-wider bg-[#161b22]">
                        {group}
                      </div>
                      {cats.map(c => (
                        <button
                          ...
                          className={`w-full text-left px-3 py-1.5 text-xs transition-colors flex items-center gap-2 ${
                            category === c ? 'bg-blue-500/15 text-blue-400' : 'text-slate-400 hover:bg-[#21262d] hover:text-slate-300'
                          }`}
                        >
            ...
            <div className="relative" ref={levelRef}>
              <button
                ...
                className="flex items-center gap-2 px-3 py-1.5 bg-[#21262d] border border-[#30363d] rounded-lg text-xs font-medium text-slate-300 hover:border-[#484f58] transition-colors min-w-[100px]"
              >
                  ...
                  {LEVELS.map(l => (
                    <button
                      ...
                      className={`w-full text-left px-3 py-2 text-xs font-medium capitalize transition-colors ${
                        level === l ? 'bg-blue-500/15 text-blue-400' : 'text-slate-400 hover:bg-[#21262d] hover:text-slate-300'
                      }`}
                    >
                      {l === 'all' ? 'All Levels' : l}
                    </button>
            ...
            <div className="relative" ref={timeRef}>
              <button
                ...
                className="flex items-center gap-2 px-3 py-1.5 bg-[#21262d] border border-[#30363d] rounded-lg text-xs font-medium text-slate-300 hover:border-[#484f58] transition-colors"
              >
                <Clock size={12} />
                ...
                <div className="absolute top-full left-0 mt-1 w-28 bg-[#1c2128] border border-[#30363d] rounded-xl shadow-2xl z-50 overflow-hidden">
                  {TIME_RANGES.map(t => (
                    <button
                      ...
                      className={`w-full text-left px-3 py-2 text-xs font-medium transition-colors ${
                        timeRange === t.key ? 'bg-blue-500/15 text-blue-400' : 'text-slate-400 hover:bg-[#21262d] hover:text-slate-300'
                      }`}
                    >
                      {t.label}
                    </button>
            ...
            <div className="relative flex-1 max-w-sm">
              ...
              <input
                ...
                className="w-full pl-9 pr-3 py-1.5 text-xs bg-[#0d1117] border border-[#30363d] rounded-lg text-slate-300 placeholder:text-slate-600 focus:outline-none focus:border-blue-500/50 focus:ring-1 focus:ring-blue-500/20 font-mono transition-colors"
              />
            </div>
        ...
        <div className="max-w-7xl mx-auto" ref={logContainerRef}>
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center h-64 text-slate-500">
              ...
              <p className="text-xs text-slate-600 mt-1">Adjust your filters or wait for new events</p>
            </div>
          ) : (
              ...
              {logs.map((log, idx) => {
                ...
                return (
                  <div key={log.id}>
                    ...
                    >
                      {/* Timestamp */}
                      <span className="text-slate-600 w-[72px] flex-shrink-0 select-none text-[12px]">
                        {timeStr}
                      </span>
                      ...
                      {/* Duration */}
                      {log.duration_ms && (
                        <span className="text-slate-600 text-[11px] ml-3 flex-shrink-0">
                          {log.duration_ms}ms
                        </span>
                      )}
                      ...
                      {/* Expand indicator */}
                      {hasDetails && (
                        <span className="text-slate-600 ml-2 flex-shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
                    ...
                    {isExpanded && hasDetails && (
                      <div className="bg-[#0d1117] border-b border-[#1c2128] px-5 py-3 ml-[128px]">
                        <pre className="text-[12px] text-slate-400 overflow-x-auto leading-relaxed whitespace-pre-wrap">
                          {JSON.stringify(log.details, null, 2)}
                        </pre>
                        {log.call_id && (
                          <div className="mt-2 flex items-center gap-2 text-[11px] text-slate-600">
                            <span>Call ID:</span>
                            <code className="bg-[#161b22] px-2 py-0.5 rounded text-slate-400">{log.call_id}</code>
                          </div>
                        )}
                        {log.source && (
                          <div className="mt-1 flex items-center gap-2 text-[11px] text-slate-600">
                            <span>Source:</span>
                            <code className="bg-[#161b22] px-2 py-0.5 rounded text-slate-400">{log.source}</code>
                          </div>
                        )}
                      </div>
                    )}
